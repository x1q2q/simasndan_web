@extends('layouts.app')
@section('extrahead')
    <title>Simasndan Web Apps - Data Semester Page</title>
    <meta name="description" content="Dashboard Page Sistem Informasi Manajemen Santri Al-Windan" />
    <meta name="_token" content="{{csrf_token()}}" />
@endsection
@section('content')
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">umum /</span> Data Semester </h4>
    ...
    <div class="card-header row justify-content-start">
        <div class="col-auto">
            <button type="button" class="btn btn-dark rounded-3" onclick="addData()"
                ...
                Tambah Semester <i class="bx bx-sm bxs-plus-circle"></i>
            </button>
        </div>
    </div>
    ...
    <div class="table-responsive text-wrap">
        <table class="table table-striped table-responsive" id="datatable-semester">
        <thead>
            <tr>
            <th>No.</th>
            <th>Semester</th>
            ...
            <div class="modal-body p-4 pb-0">
                ...
                <div class="row">
                    ...
                    <div class="input-group">
                        <span class="input-group-text" id="basic-addon14">Semester ke-</span>
                        <input type="number" class="form-control" placeholder="cth:1" id="semester" name="semester">
                    </div>                    
                    </div>
                </div>
                <div class="row">
...
<script src="{{ asset('assets/js/ui-modals.js') }}"></script>
    ...
    @endif
<script type="text/javascript">
    var urlPhoto = "{{ asset('assets/img/uploads/semester') }}";
    $(document).ready(function(){
        var dtableSemester = $('#datatable-semester').DataTable({
            "searching": false,
            "autoWidth":false,
            "lengthChange": true,
            "pagingType": "simple",
        ...
        dtableSemester.on( 'draw', function () {
            var total_records = dtableSemester.rows().count();
            var page_length = dtableSemester.page.info().length;
            var total_pages = Math.ceil(total_records / page_length);
            var current_page = dtableSemester.page.info().page+1;
        });
        ...
        $('#form-save').submit(function(e){
                e.preventDefault();
                let siteUrl = "{{ route('semester') }}";
                let formData = new FormData(this);
                let idsemester = $('#form-save').find('#id_semester').val();
    ...
    function hapusData(id){
        let urlDelete = "{{ route('semester.delete',':id')}}";
